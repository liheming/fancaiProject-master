...
public class CirclePostAdapter extends RecyclerView.Adapter<CirclePostAdapter.MyViewHolder> {
   public static  final    String TAG = "CirclePostAdapter";
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        final TopicCommentBean topicCommentBean = topicCommentList.get(position);
...
//        Log.i(TAG, "onBindViewHolder: "+TokenManager.getToken()+"1111"+topicCommentBean.getUser_token());
        if (TokenManager.getToken().equals(topicCommentBean.getUser_token())) {

//            Log.i(TAG, "onBindViewHolder: 相等");
            holder1.imvb_comment_delete.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {

                    topicInterface.onDeleteCommentClick(position);
//            if (topicCommentBean.ge)
                }
            });
        } else {
            holder1.imvb_comment_delete.setVisibility(View.GONE);
//            Log.i(TAG, "onBindViewHolder:  不相等");
        }
     ...
     *adapter数据更新方法
     */
    public void setDataChange(ArrayList<TopicCommentBean> topicCommentList) {
        this.topicCommentList = topicCommentList;
    ...
    public void onTopicOnclick(TopicInterface topicInterface) {
