...
import com.suctan.huigang.adapter.order.sellorder.SellDetailItemAdapter;
...
public class SellOrderDetailActivity extends Activity implements View.OnClickListener {
    ...
    private SellOrderBean mSellOrder;
    ...
    private void getIntentData() {
        ...
        mSellOrder = (SellOrderBean) deliverIntent.getSerializableExtra("sell");
    ...
    private void initViewData() {
        tv_orderdetail_nubmer.setText(mSellOrder.getId() + "");
        tv_orderdetail_note.setText(mSellOrder.getOrder_note());
        tv_orderdetail_username.setText(mSellOrder.getUser_name());
        if (orderSortKey == 0) {
            tv_orderdetail_allprice.setText(mSellOrder.getAll_price() + "");
            ArrayList<WantEatFoodItem> wantEatFoodItemList = new ArrayList<>();
            wantEatFoodItemList.add(new WantEatFoodItem(mSellOrder.getOrder_title(), -1, mSellOrder.getNum()));
            initAdateper(wantEatFoodItemList);
        } else {
            tv_orderdetail_allprice.setText(mSellOrder.getOrder_price() + "");
            initAdateper(culToObject(mSellOrder.getEatstrarr()));
        }
        String type = null;
        if (mSellOrder.getOrder_type() == 0) {
            type = "上门做";
        } else if (mSellOrder.getOrder_type() == 1) {
            type = "送上门";
        } else {
            type = "自提";
        }
        tv_ordertail_servicetype.setText(type);
        tv_ordertail_address.setText(mSellOrder.getUser_address());
        tv_ordertail_phone.setText(mSellOrder.getUser_phone());
        tv_dowant_expecttime.setText(mSellOrder.getOrder_expect_time());
    }

    private void initAdateper(ArrayList<WantEatFoodItem> wantEatFoodItems) {
        SellDetailItemAdapter adapter = new SellDetailItemAdapter(this, wantEatFoodItems);
