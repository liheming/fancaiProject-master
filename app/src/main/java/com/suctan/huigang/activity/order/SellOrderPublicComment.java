...
public class SellOrderPublicComment extends MvpActivity<MyBuyOrderPresenter> implements View.OnClickListener, MyBuyOrderView {
    ...
    private void getIntentData() {
        ...
        tv_cookCommend_title.setText(sellOrderBean.getOrder_title());
        System.out.println("标题是" + sellOrderBean.getOrder_title());
        tv_cookCommend_time.setText(sellOrderBean.getOrder_expect_time());
        System.out.println("时间是" + sellOrderBean.getOrder_expect_time());
        if (sellOrderBean != null) {
            Map<String, Object> map = new HashMap();
            map.put("user_token", TokenManager.getToken());
            map.put("order_id", sellOrderBean.getOrder_id());
            mvpPresenter.SellCheckOrderCommentReturn(map);
        }
    }
    ...
    private void initView() {
        ...
        tv_cookCommend_title = (TextView) findViewById(R.id.tv_food_title);
    ...
    private void initViewData(BuyPCommendReturn buyPCommendReturn) {
        if (buyPCommendReturn.getCusComment() != null) {
            ...
            System.out.println("客户的姓名是"+buyCusComment.getUser_name());
            ...
            if (TextUtils.isEmpty(buyCusComment.getOrder_comment())) {
                tv_cookCommend_content.setText("买家暂未对该定单进行评论！");
            } else {
                tv_cookCommend_content.setText(buyCusComment.getOrder_comment());
            }
        }

        if (buyPCommendReturn.getCookComment() != null) {
            BuyPCommendCookBean buyPcommendCook = buyPCommendReturn.getCookComment();
            System.out.println("厨师的姓名是"+buyPcommendCook.getUser_name());
            ...
            if (TextUtils.isEmpty(buyPcommendCook.getOrder_cookcomment())) {
                tv_CurComment_content.setText("您暂未对该定单进行评论！");
