package com.suctan.huigang.mvp.login.buyorder;
...
public class MyBuyOrderPresenter extends DemoBasePresenter<MyBuyOrderView> {
        ...
        addSubscription(apiStores.getBuyAllOrderListReturn(map),
                new SubscriberCallBack<>(new ApiCallback<String>() {
                    ...
                    public void onSuccess(String model) {
                        ...
                        ArrayList<BuyRecommendBean> list = new ArrayList<>();
                        list.addAll(buyRecommendReturn.getRecommendBeenList());
    ...
    public final static String TAG = "MyBuyOrderPresenter";
    /**
     * 我要吃厨师查看评论
     *
     * @param
     ...
     * @param
     */
    public void SellCheckOrderCommentReturn(Map<String, Object> map) {
        addSubscription(apiStores.SellCheckOrderCommentReturn(map),
                new SubscriberCallBack<>(new ApiCallback<BuyPCommendReturn>() {
                    @Override
                    public void onStart() {
                        Log.i(TAG, "onStart: ");
        ...
        addSubscription(apiStores.BuyCheckAOrderCommentReturn(map),
                new SubscriberCallBack<>(new ApiCallback<BuyACommendReturn>() {
                    ...
                    public void onSuccess(BuyACommendReturn model) {
                        ...
                        System.out.println("查看评论" + model.toString());
                        if (model.getStatus() == 1) {
                            BuyACommendBean by = model.getComment();
                            Log.i(TAG, "onSuccess: "+by.getComment_describe()+by.getComment_content()+by.getComment_service()+by.getComment_taste());
                            Log.i(TAG, "onSuccess: ");
