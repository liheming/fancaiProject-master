...
import com.suctan.huigang.adapter.order.buyorder.FinishRecommdOrederAdapter;
...
public class MyPublishFinishOrder extends MvpFragment<MyBuyOrderPresenter> implements MyBuyOrderView, FinishRecommdOrederAdapter.FinishDetailOnClickListener {
    ...
    public void getFinishRecommderOrderSuc(ArrayList<BuyRecommendBean> finishRecommendOrder) {
            finishRecommendBeenList.clear();//清除之前的数据
        if (isFresh) {
            isFresh = false;
        }
    ...
    private void initAdapter(ArrayList<BuyRecommendBean> finishRecommendBeenList) {
        finishListView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            @Override
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ToastTool.showToast("长按删除该订单position" + position, 2);
//                deleteFood(position);
                // FIXME: 2017/5/3 此处做长按删除该订单
                return true;
            }
        });
    ...
    public void onItemOnClick(int position) {
        ...
        intent.putExtra("buy", finishRecommendBeenList.get(position));
        intent.putExtra("buySort", 1);
        getActivity().startActivity(intent);
    }

    @Override
    public void onCheckComments(int position) {
        Intent intent = new Intent(getActivity(), BuyOrderPublicComment.class);
        intent.putExtra("pComment", finishRecommendBeenList.get(position));
