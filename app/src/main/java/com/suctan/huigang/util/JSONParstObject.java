package com.suctan.huigang.util;


import android.util.Log;
import com.suctan.huigang.bean.address.AddAdressReturn;
...
import com.suctan.huigang.bean.dowant.DoWantOrderReturn;
...
import com.suctan.huigang.bean.order.buy.BuyRecommendReturn;
import com.suctan.huigang.bean.order.sell.SellOrderBean;
import com.suctan.huigang.bean.order.sell.SellOrderReturn;
...
import com.suctan.huigang.bean.user.MyAlreadykitchenBean;
import com.suctan.huigang.bean.user.MyAlreadykitchenReturn;
import com.suctan.huigang.bean.user.MykitchenBean;
import com.suctan.huigang.bean.user.MykitchenReturn;
import com.suctan.huigang.bean.user.Result;
import com.suctan.huigang.bean.user.Users;
import com.suctan.huigang.bean.wanteat.WantEatOtderReturn;
...
public class JSONParstObject {
    public static final String TAG = "JSONParstObject";
...
//            result.setStatus(jsonObject.getInt("status"));
            ...
            Log.i(TAG, "GetUserJSonObject: ");
            System.out.println(user.getString("user_phone"));

            currentUser = new Users();
            if (user.getString("user_phone") != null) {
                currentUser.setUser_phone(user.getString("user_phone"));
            }
            if (user.getString("user_name") != null) {
                currentUser.setUser_name(user.getString("user_name"));
            }
            if (user.getString("user_icon") != null) {
                currentUser.setUser_icon(user.getString("user_icon"));
            }
            if (user.getString("user_alias") != null) {
                currentUser.setUser_alias(user.getString("user_alias"));
            }
            if (user.getString("user_sex") != null) {
                int sex = Integer.parseInt(user.getString("user_sex"));
                currentUser.setUser_sex(sex);
            }
            if (user.getString("user_age") != null) {
                currentUser.setUser_age(user.getString("user_age"));
            }
            if (user.getString("user_education") != null) {
                currentUser.setUser_education(user.getString("user_education"));
            }
            if (user.getString("user_skill") != null) {
                currentUser.setUser_skill(user.getString("user_skill"));
            }
            if (user.getString("user_hobby") != null) {
                currentUser.setUser_hobby(user.getString("user_hobby"));
            }
            if (user.getString("user_address") != null) {
                currentUser.setUser_address(user.getString("user_address"));
            }
            currentUser.setUser_money(user.getDouble("user_money"));

            if (user.getString("user_card") != null) {
                currentUser.setUser_card(user.getString("user_card"));
            }
            if (user.getString("user_coupon") != null) {
                currentUser.setUser_coupon(user.getString("user_coupon"));
            }
            currentUser.setIs_cooking(user.getInt("is_cooking"));

            if (user.getString("longitude") != null) {
                double longitude = Double.parseDouble(user.getString("longitude"));
                currentUser.setLongitude(longitude);
            }
            if (user.getString("latitude") != null) {
        ...
        try {
            ...
            if (jsonArray != null) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    eatFoodBean.setOrder_pic(jsonObject1.getString("order_pic"));
                    eatFoodBean.setOrder_title(jsonObject1.getString("order_title"));
                    String orderPrice = jsonObject1.getString("order_price");


                    if (jsonObject1.getString("longitude") != null) {
                        double longitude = Double.parseDouble(jsonObject1.getString("longitude"));
                        eatFoodBean.setLongitude(longitude);
                    }
                    if (jsonObject1.getString("latitude") != null) {
                        double latitude = Double.parseDouble(jsonObject1.getString("latitude"));
                        eatFoodBean.setLatitude(latitude);
                    }

                    if (jsonObject1.getString("star") != null) {
                        eatFoodBean.setStar(Integer.parseInt(jsonObject1.getString("star")));
                    }
                    ...
                    if (jsonObject1.getString("order_num") != null) {
        ...
        try {
            ...
            if (jsonArray != null) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    topicCommentBean.setUser_token(jsonObject1.getString("user_token"));
                    topicCommentBean.setUser_icon(jsonObject1.getString("user_icon"));
        ...
        try {
            ...
            if (jsonArrayOther != null) {
                for (int i = 0; i < jsonArrayOther.length(); i++) {
                    ...
                    eatFoodBean.setOrder_pic(jsonObjectOther.getString("order_pic"));
                    ...
                    if (jsonObjectOther.getString("longitude") != null) {
                        double longitude = Double.parseDouble(jsonObjectOther.getString("longitude"));
                        eatFoodBean.setLongitude(longitude);
                    }
                    if (jsonObjectOther.getString("latitude") != null) {
                        double latitude = Double.parseDouble(jsonObjectOther.getString("latitude"));
                        eatFoodBean.setLatitude(latitude);
                    }

                    if (jsonObjectOther.getString("star") != null) {
                        eatFoodBean.setStar(Integer.parseInt(jsonObjectOther.getString("star")));
                    }
            ...
            if (jsonArrayComment != null) {
                for (int j = 0; j < jsonArrayComment.length(); j++) {
                    ...
                    wantEatOtherComment.setUser_icon(jsonComment.getString("user_icon"));
...
//    public static SellOrderReturn getMakeOrderList(String addressList, int status) {
...
//
//
//                    sellOrderBean.setPic(jsonObject1.getString("pic"));
    ...
    public static BuyRecommendReturn getMakeOrderList(String addressList, int status) {
        ...
        try {
            ...
            if (jsonArray != null) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject jsonObject1 = (JSONObject) jsonArray.get(i);
                    BuyRecommendBean sellOrderBean = new BuyRecommendBean();
                    String order_status = jsonObject1.getString("status");
                    if (order_status != null) {
                        sellOrderBean.setOrder_status(Integer.parseInt(order_status));
                    }
                    ...
                    sellOrderBean.setUser_name(jsonObject1.getString("user_name"));
                    sellOrderBean.setUser_phone(jsonObject1.getString("user_phone"));
                    sellOrderBean.setUer_address(jsonObject1.getString("user_address"));

                    buyRecommendBeenList.add(sellOrderBean);
...
* */
    ...
    public static MyAlreadykitchenReturn getMyAlreadykitchenBeanList(String MakeOrderList) {
        MyAlreadykitchenReturn myAlreadykitchenReturn = null;
        ArrayList<MyAlreadykitchenBean> mykitchenBeenList = null;
        try {
            JSONObject jsonObject = new JSONObject(MakeOrderList);
            myAlreadykitchenReturn = new MyAlreadykitchenReturn();
            mykitchenBeenList = new ArrayList<>();
            myAlreadykitchenReturn.setStatus(jsonObject.getInt("status"));
            myAlreadykitchenReturn.setMsg(jsonObject.getString("msg"));
            JSONArray jsonArray = null;
            jsonArray = jsonObject.getJSONArray("myMakeOrderList");
            if (jsonArray != null) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject jsonObject1 = (JSONObject) jsonArray.get(i);
                    MyAlreadykitchenBean mykitchenBean = new MyAlreadykitchenBean();

                    /*mykitchenBean.setOrder_id(jsonObject1.getInt("order_id"));*/
                    mykitchenBean.setOrder_title(jsonObject1.getString("order_title"));
                    mykitchenBean.setFood_description(jsonObject1.getString("food_description"));
                    mykitchenBean.setOrder_price(jsonObject1.getDouble("order_price"));
                    mykitchenBean.setOrder_pic(jsonObject1.getString("order_pic"));

                    mykitchenBeenList.add(mykitchenBean);
                }
                myAlreadykitchenReturn.setMykitchenBeanList(mykitchenBeenList);
            }
        } catch (JSONException e) {
            e.printStackTrace();
            return null;
        }
        return myAlreadykitchenReturn;
    }


    public static MykitchenReturn getMykitchenBeanList(String MakeOrderList, int status) {
        ...
        try {
            ...
            switch (status) {
                ...
                case 2:
                    jsonArray = jsonObject.getJSONArray("todayOrderList");
                    break;
                case 3:

                    jsonArray = jsonObject.getJSONArray("unloadOrderList");
                    break;
            }
            ...
            if (jsonArray != null) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    String order_id = jsonObject1.getString("order_id");
                    if (order_id != null) {
                        mykitchenBean.setOrder_id(Integer.parseInt(order_id));
                    }
                    String order_status = jsonObject1.getString("order_status");
                    if (order_status != null) {
                        mykitchenBean.setOrder_status(Integer.parseInt(order_status));
                    }
                    String order_num = jsonObject1.getString("order_num");
                    if (order_num != null) {
                        mykitchenBean.setOrder_num(Integer.parseInt(order_num));
                    }
                    String order_type = jsonObject1.getString("order_type");
                    if (order_type != null) {
                        mykitchenBean.setOrder_type(Integer.parseInt(order_type));
                    }
        ...
        try {
            ...
            if (jsonArray != null) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    String need_status = jsonObject1.getString("need_status");
                    if (need_status != null) {
                        needBean.setNeed_status(Integer.parseInt(need_status));
                    }
                    if (jsonObject1.getString("latitude") != null) {
                        double latitude = Double.parseDouble(jsonObject1.getString("latitude"));
                        needBean.setLatitude(latitude);
                    }
                    if (jsonObject1.getString("longitude") != null) {
                        double longitude = Double.parseDouble(jsonObject1.getString("longitude"));
                        needBean.setLongitude(longitude);
                    }
